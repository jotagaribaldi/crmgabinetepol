...
// Dados completos do IBGE - Estados brasileiros
const STATES = [
...
async function fetchMunicipalities() {
  ...
  const data = (await response.json()) as Array<{
    id: number;
    nome: string;
    microrregiao?: { mesorregiao?: { UF?: { id: number; sigla: string } } } | null;
    'regiao-imediata'?: {
      'regiao-intermediaria'?: { UF?: { id: number; sigla: string } } | null;
    } | null;
  }>;
  return data;
}

function extractStateIbgeCode(m: any): string | null {
  // Tenta microrregiao → mesorregiao → UF
  try {
  ...
  // Último fallback: extrair do ibgeCode do município (2 primeiros dígitos)
